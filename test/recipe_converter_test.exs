defmodule RecipeConverterTest do
  ...
  def final1 do
    Convert.convert("https://cooking.nytimes.com/recipes/1017348-the-best-roast-beef-for-sandwiches?action=click&module=Global%20Search%20Recipe%20Card&pgType=search&rank=10", case1())
  end
  ...
  test "Fields that shouldn't be changed by Convert.convert remain unchanged" do
    assert "The Best Roast Beef for Sandwiches" == final1().name
    assert "Melissa Clark" == final1().inspiration
    assert "https://cooking.nytimes.com/recipes/1017348-the-best-roast-beef-for-sandwiches?action=click&module=Global%20Search%20Recipe%20Card&pgType=search&rank=10"
      == final1().inspiration_url
  end
